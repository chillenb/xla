...
#ifndef XLA_SERVICE_CPU_RUNTIME_BLIS_MATMUL_COMMON_H_
#define XLA_SERVICE_CPU_RUNTIME_BLIS_MATMUL_COMMON_H_
...
void BlisMatMul(const void* run_options_ptr, double* out, double* lhs, double* rhs, int64_t m,
            int64_t n, int64_t k, int32_t transpose_lhs,
            int32_t transpose_rhs) {
  const xla::ExecutableRunOptions* run_options =
      static_cast<const xla::ExecutableRunOptions*>(run_options_ptr);

  const int64_t rsa = transpose_lhs ? k : 1;
  const int64_t csa = transpose_lhs ? 1 : m;

  const int64_t rsb = transpose_rhs ? n : 1;
  const int64_t csb = transpose_rhs ? 1 : k;
  ...
  trans_t transa = transpose_lhs ? BLIS_TRANSPOSE : BLIS_NO_TRANSPOSE;
  trans_t transb = transpose_rhs ? BLIS_TRANSPOSE : BLIS_NO_TRANSPOSE;
...
*/
  ...
  bli_dgemm_ex(transa, transb, m, n, k, 1.0,
               lhs, rsa, csa,
               rhs, rsb, csb,
               0.0,
               out, 1, n,
